import { PaginatedMessage, PaginatedFieldMessageEmbed } from '@sapphire/discord.js-utilities';
import { ApplyOptions } from '@sapphire/decorators';
import { ApplicationCommandRegistry, Command, CommandOptions, container } from '@sapphire/framework';
import { ApplicationCommandOption, AutocompleteInteraction, CommandInteraction, MessageEmbed } from 'discord.js';

@ApplyOptions<CommandOptions>({
	name: 'help',
	description: "Get more info about this bot's commands."
})
export class HelpCommand extends Command {
	...
	public override async chatInputRun(interaction: CommandInteraction) {
		...
		if (!query) {
			...
			sortedList.reduce((_obj, command) => commandMap.set(command.name, command), {});
			if (commandMap.has(query)) {
				...
				const commandDetails = new MessageEmbed()
					...
					.setThumbnail(app?.iconURL()!)
					.setTitle(`${command.name.charAt(0).toUpperCase() + command.name.slice(1).toLowerCase()} - Details`)
					.setColor('#9096e6')
					.setDescription(`**Description**\n> ${command.details}`);

				if (!command.options.length) return await interaction.reply({ embeds: [commandDetails] });

				DetailedPagination.setTemplate(commandDetails)
					.setTitleField('Options')
					.setItems(command.options)
					.formatItems((option: any) => `**${option.name}**\n> ${option.description}`)
					.setItemsPerPage(5)
					.make();
			...
			options: [
				{
					...
					required: false,
					name: 'command',
					description: 'Which command would you like to know more about?',
					autocomplete: true
				}
			]
		});
