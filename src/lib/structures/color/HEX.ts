...
export class HEX implements ColorHandler {
	...
	public check() {
		if (Number.isNaN(parseInt(this.r, 16))) {
			throw new UserError({ identifier: 'Invalid Red Value', context: { value: this.r } });
		}
		if (Number.isNaN(parseInt(this.g, 16))) {
			throw new UserError({ identifier: 'Invalid Green Value', context: { value: this.g } });
		}
		if (Number.isNaN(parseInt(this.b, 16))) {
			throw new UserError({ identifier: 'Invalid Blue Value', context: { value: this.b } });
		}
	}
